...
public class UserDaoImpl implements UserDao {
    ...
    private static final String GET_BY_STATUS = "SELECT * from user where status=?";
    ...
    private static final String CREATE_USER = "INSERT into user (first_name, second_name, patronymic, email, password, status, id_role, phone_number) " +
            "values (?, ?, ?,?, ?, ?, ?, ?); ";
    ...
    public User getByEmail(String email) throws SQLException, ClassNotFoundException {
        ...
        while (rs.next()) {
            User user = new User(rs.getLong("id"), rs.getString("first_name"), rs.getString("second_name"), rs.getString("patronymic"),
                    email, rs.getString("password"), BlockStatus.valueOf(rs.getString("status")),
                    rs.getInt("id_role"), rs.getString("phone_number"));
            rs.close();
            ps.close();
            con.close();
            return user;
    ...
    public User getById(Long id) throws SQLException {
        ...
        while (rs.next()) {
            user = new User(id, rs.getString("first_name"), rs.getString("second_name"), rs.getString("patronymic"),
                    rs.getString("email"), rs.getString("password"), BlockStatus.valueOf(rs.getString("status")),
                    rs.getInt("id_role"), rs.getString("phone_number"));
        }
        rs.close();
        ps.close();
    ...
    public List<User> getByStatus(String status) throws SQLException {
        ...
        while (rs.next()) {
            User user = new User(rs.getLong("id"), rs.getString("first_name"), rs.getString("second_name"), rs.getString("patronymic"),
                    rs.getString("email"), rs.getString("password"), BlockStatus.valueOf(status),
                    rs.getInt("id_role"), rs.getString("phone_number"));
            userList.add(user);
        }
        rs.close();
